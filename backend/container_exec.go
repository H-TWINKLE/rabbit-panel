...
func parseLsOutput(output string, basePath string) []FileInfo {
	...
	for _, line := range lines {
		...
		// 处理符号链接：名称中包含 " -> " 表示是符号链接
		// 例如：bin -> usr/bin
		isSymlink := strings.Contains(name, " -> ")
		if isSymlink {
			// 只保留链接名，去掉目标路径
...
func handleContainerUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if req.Memory > 0 {
		// 前端传的是 MB，转换为字节
		memoryBytes := req.Memory * 1024 * 1024
		updateConfig.Memory = memoryBytes
		// 设置 MemorySwap 为 Memory 的 2 倍
		updateConfig.MemorySwap = memoryBytes * 2
	}
...
func handleContainerRecreate(w http.ResponseWriter, r *http.Request) {
	...
	// 1. 先检查容器状态，只有运行中的容器才需要停止
	containerInfo, err := dockerClient.ContainerInspect(ctx, req.ContainerID)
	if err != nil {
		http.Error(w, "获取容器信息失败: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func handleContainerTerminalWS(w http.ResponseWriter, r *http.Request) {
	...
	// 验证 token（WebSocket 不能使用 authMiddleware，需要手动验证）
	...
	conn.SetCloseHandler(func(code int, text string) error {
		log.Printf("[Terminal] WebSocket close handler: code=%d, text=%s", code, text)
		return nil
	})
		...
		for {
			...
			if messageType == websocket.TextMessage && len(message) > 0 && message[0] == '{' {
				...
				if err := json.Unmarshal(message, &resizeMsg); err == nil && resizeMsg.Type == "resize" {
					// 调整终端大小
					if err := dockerClient.ContainerExecResize(ctx, execID.ID, container.ResizeOptions{
