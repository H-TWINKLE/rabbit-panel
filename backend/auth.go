...
func generateNodeToken(nodeID string) string {
	...
	// 按天生成 token，避免时间差问题
	h.Write([]byte(nodeID + ":" + time.Now().Format("2006-01-02")))
	return hex.EncodeToString(h.Sum(nil))
}

// 验证节点 Token
func verifyNodeToken(nodeID, token string) bool {
	// 验证今天、昨天、明天的 token（允许跨天和时区差异）
	now := time.Now()
	for i := -1; i <= 1; i++ {
		t := now.AddDate(0, 0, i)
	...
	log.Printf("[NodeAuth] Token 验证失败: NodeID=%s, 收到的Token=%s...", nodeID, token[:16])
	return false
}
...
func generateNodeTokenForTime(nodeID string, t time.Time) string {
	h := hmac.New(sha256.New, []byte(nodeSecret))
	// 按天生成
	h.Write([]byte(nodeID + ":" + t.Format("2006-01-02")))
	return hex.EncodeToString(h.Sum(nil))
}
...
func nodeAuthMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if nodeID == "" || nodeToken == "" {
			log.Printf("[NodeAuth] 认证失败: 缺少节点ID或Token, NodeID=%s, HasToken=%v", nodeID, nodeToken != "")
		...
		if !verifyNodeToken(nodeID, nodeToken) {
			log.Printf("[NodeAuth] 认证失败: Token无效, NodeID=%s", nodeID)
			http.Error(w, `{"error": "节点认证失败: Token无效"}`, http.StatusUnauthorized)
			return
		}
		
		log.Printf("[NodeAuth] 认证成功: NodeID=%s", nodeID)
		next(w, r)
	}
}
...
// validateToken 简单验证 token 是否有效（供 WebSocket 等场景使用）
